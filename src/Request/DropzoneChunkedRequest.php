...
class DropzoneChunkedRequest
{
    ...
    private function fetchStringRequestInput(Request $request, string $key): string
    {
        ...
        if (!\is_string($input)) {
            throw new BadDropzoneRequest(\sprintf('The "%s" request input key is missing.', $key));
        }
    ...
    {
        $input = $request->request->filter($key, filter: \FILTER_VALIDATE_INT, options: ['flags' => \FILTER_NULL_ON_FAILURE]);
    ...
    private function fetchRequestFile(Request $request, string $key): UploadedFile
    {
        ...
        if (!$file instanceof UploadedFile) {
            throw new BadDropzoneRequest(\sprintf('The "%s" request file is missing.', $key));
        }
